...
function cudaDeviceReset()
    ccall((:cudaDeviceReset, libcudnn), cudaError_t, ())
end

function cudaDeviceSynchronize()
    ccall((:cudaDeviceSynchronize, libcudnn), cudaError_t, ())
end

function cudaDeviceSetLimit(limit::cudaLimit, value::Cint)
    ccall((:cudaDeviceSetLimit, libcudnn), cudaError_t, (cudaLimit, Cint), limit, value)
end

function cudaDeviceGetLimit(pValue, limit::cudaLimit)
    ccall((:cudaDeviceGetLimit, libcudnn), cudaError_t, (Ptr{Cint}, cudaLimit), pValue, limit)
end

function cudaDeviceGetCacheConfig(pCacheConfig)
    ccall((:cudaDeviceGetCacheConfig, libcudnn), cudaError_t, (Ptr{cudaFuncCache},), pCacheConfig)
end

function cudaDeviceGetStreamPriorityRange(leastPriority, greatestPriority)
    ccall((:cudaDeviceGetStreamPriorityRange, libcudnn), cudaError_t, (Ptr{Cint}, Ptr{Cint}), leastPriority, greatestPriority)
end

function cudaDeviceSetCacheConfig(cacheConfig::cudaFuncCache)
    ccall((:cudaDeviceSetCacheConfig, libcudnn), cudaError_t, (cudaFuncCache,), cacheConfig)
end

function cudaDeviceGetSharedMemConfig(pConfig)
    ccall((:cudaDeviceGetSharedMemConfig, libcudnn), cudaError_t, (Ptr{cudaSharedMemConfig},), pConfig)
end

function cudaDeviceSetSharedMemConfig(config::cudaSharedMemConfig)
    ccall((:cudaDeviceSetSharedMemConfig, libcudnn), cudaError_t, (cudaSharedMemConfig,), config)
end

function cudaDeviceGetByPCIBusId(device, pciBusId)
    ccall((:cudaDeviceGetByPCIBusId, libcudnn), cudaError_t, (Ptr{Cint}, Cstring), device, pciBusId)
end

function cudaDeviceGetPCIBusId(pciBusId, len::Cint, device::Cint)
    ccall((:cudaDeviceGetPCIBusId, libcudnn), cudaError_t, (Cstring, Cint, Cint), pciBusId, len, device)
end

function cudaIpcGetEventHandle(handle, event::cudaEvent_t)
    ccall((:cudaIpcGetEventHandle, libcudnn), cudaError_t, (Ptr{cudaIpcEventHandle_t}, cudaEvent_t), handle, event)
end

function cudaIpcOpenEventHandle(event, handle::cudaIpcEventHandle_t)
    ccall((:cudaIpcOpenEventHandle, libcudnn), cudaError_t, (Ptr{cudaEvent_t}, cudaIpcEventHandle_t), event, handle)
end

function cudaIpcGetMemHandle(handle, devPtr)
    ccall((:cudaIpcGetMemHandle, libcudnn), cudaError_t, (Ptr{cudaIpcMemHandle_t}, Ptr{Void}), handle, devPtr)
end

function cudaIpcOpenMemHandle(devPtr, handle::cudaIpcMemHandle_t, flags::UInt32)
    ccall((:cudaIpcOpenMemHandle, libcudnn), cudaError_t, (Ptr{Ptr{Void}}, cudaIpcMemHandle_t, UInt32), devPtr, handle, flags)
end

function cudaIpcCloseMemHandle(devPtr)
    ccall((:cudaIpcCloseMemHandle, libcudnn), cudaError_t, (Ptr{Void},), devPtr)
end

function cudaThreadExit()
    ccall((:cudaThreadExit, libcudnn), cudaError_t, ())
end

function cudaThreadSynchronize()
    ccall((:cudaThreadSynchronize, libcudnn), cudaError_t, ())
end

function cudaThreadSetLimit(limit::cudaLimit, value::Cint)
    ccall((:cudaThreadSetLimit, libcudnn), cudaError_t, (cudaLimit, Cint), limit, value)
end

function cudaThreadGetLimit(pValue, limit::cudaLimit)
    ccall((:cudaThreadGetLimit, libcudnn), cudaError_t, (Ptr{Cint}, cudaLimit), pValue, limit)
end

function cudaThreadGetCacheConfig(pCacheConfig)
    ccall((:cudaThreadGetCacheConfig, libcudnn), cudaError_t, (Ptr{cudaFuncCache},), pCacheConfig)
end

function cudaThreadSetCacheConfig(cacheConfig::cudaFuncCache)
    ccall((:cudaThreadSetCacheConfig, libcudnn), cudaError_t, (cudaFuncCache,), cacheConfig)
end

function cudaGetLastError()
    ccall((:cudaGetLastError, libcudnn), cudaError_t, ())
end

function cudaPeekAtLastError()
    ccall((:cudaPeekAtLastError, libcudnn), cudaError_t, ())
end

function cudaGetErrorName(error::cudaError_t)
    ccall((:cudaGetErrorName, libcudnn), Cstring, (cudaError_t,), error)
end

function cudaGetErrorString(error::cudaError_t)
    ccall((:cudaGetErrorString, libcudnn), Cstring, (cudaError_t,), error)
end

function cudaGetDeviceCount(count)
    ccall((:cudaGetDeviceCount, libcudnn), cudaError_t, (Ptr{Cint},), count)
end

function cudaGetDeviceProperties(prop, device::Cint)
    ccall((:cudaGetDeviceProperties, libcudnn), cudaError_t, (Ptr{cudaDeviceProp}, Cint), prop, device)
end

function cudaDeviceGetAttribute(value, attr::cudaDeviceAttr, device::Cint)
    ccall((:cudaDeviceGetAttribute, libcudnn), cudaError_t, (Ptr{Cint}, cudaDeviceAttr, Cint), value, attr, device)
end

function cudaDeviceGetP2PAttribute(value, attr::cudaDeviceP2PAttr, srcDevice::Cint, dstDevice::Cint)
    ccall((:cudaDeviceGetP2PAttribute, libcudnn), cudaError_t, (Ptr{Cint}, cudaDeviceP2PAttr, Cint, Cint), value, attr, srcDevice, dstDevice)
end

function cudaChooseDevice(device, prop)
    ccall((:cudaChooseDevice, libcudnn), cudaError_t, (Ptr{Cint}, Ptr{cudaDeviceProp}), device, prop)
end

function cudaSetDevice(device::Cint)
    ccall((:cudaSetDevice, libcudnn), cudaError_t, (Cint,), device)
end

function cudaGetDevice(device)
    ccall((:cudaGetDevice, libcudnn), cudaError_t, (Ptr{Cint},), device)
end

function cudaSetValidDevices(device_arr, len::Cint)
    ccall((:cudaSetValidDevices, libcudnn), cudaError_t, (Ptr{Cint}, Cint), device_arr, len)
end

function cudaSetDeviceFlags(flags::UInt32)
    ccall((:cudaSetDeviceFlags, libcudnn), cudaError_t, (UInt32,), flags)
end

function cudaGetDeviceFlags(flags)
    ccall((:cudaGetDeviceFlags, libcudnn), cudaError_t, (Ptr{UInt32},), flags)
end

function cudaStreamCreate(pStream)
    ccall((:cudaStreamCreate, libcudnn), cudaError_t, (Ptr{cudaStream_t},), pStream)
end

function cudaStreamCreateWithFlags(pStream, flags::UInt32)
    ccall((:cudaStreamCreateWithFlags, libcudnn), cudaError_t, (Ptr{cudaStream_t}, UInt32), pStream, flags)
end

function cudaStreamCreateWithPriority(pStream, flags::UInt32, priority::Cint)
    ccall((:cudaStreamCreateWithPriority, libcudnn), cudaError_t, (Ptr{cudaStream_t}, UInt32, Cint), pStream, flags, priority)
end

function cudaStreamGetPriority(hStream::cudaStream_t, priority)
    ccall((:cudaStreamGetPriority, libcudnn), cudaError_t, (cudaStream_t, Ptr{Cint}), hStream, priority)
end

function cudaStreamGetFlags(hStream::cudaStream_t, flags)
    ccall((:cudaStreamGetFlags, libcudnn), cudaError_t, (cudaStream_t, Ptr{UInt32}), hStream, flags)
end

function cudaStreamDestroy(stream::cudaStream_t)
    ccall((:cudaStreamDestroy, libcudnn), cudaError_t, (cudaStream_t,), stream)
end

function cudaStreamWaitEvent(stream::cudaStream_t, event::cudaEvent_t, flags::UInt32)
    ccall((:cudaStreamWaitEvent, libcudnn), cudaError_t, (cudaStream_t, cudaEvent_t, UInt32), stream, event, flags)
end

function cudaStreamAddCallback(stream::cudaStream_t, callback::cudaStreamCallback_t, userData, flags::UInt32)
    ccall((:cudaStreamAddCallback, libcudnn), cudaError_t, (cudaStream_t, cudaStreamCallback_t, Ptr{Void}, UInt32), stream, callback, userData, flags)
end

function cudaStreamSynchronize(stream::cudaStream_t)
    ccall((:cudaStreamSynchronize, libcudnn), cudaError_t, (cudaStream_t,), stream)
end

function cudaStreamQuery(stream::cudaStream_t)
    ccall((:cudaStreamQuery, libcudnn), cudaError_t, (cudaStream_t,), stream)
end

function cudaStreamAttachMemAsync(stream::cudaStream_t, devPtr, length::Cint, flags::UInt32)
    ccall((:cudaStreamAttachMemAsync, libcudnn), cudaError_t, (cudaStream_t, Ptr{Void}, Cint, UInt32), stream, devPtr, length, flags)
end

function cudaEventCreate(event)
    ccall((:cudaEventCreate, libcudnn), cudaError_t, (Ptr{cudaEvent_t},), event)
end

function cudaEventCreateWithFlags(event, flags::UInt32)
    ccall((:cudaEventCreateWithFlags, libcudnn), cudaError_t, (Ptr{cudaEvent_t}, UInt32), event, flags)
end

function cudaEventRecord(event::cudaEvent_t, stream::cudaStream_t)
    ccall((:cudaEventRecord, libcudnn), cudaError_t, (cudaEvent_t, cudaStream_t), event, stream)
end

function cudaEventQuery(event::cudaEvent_t)
    ccall((:cudaEventQuery, libcudnn), cudaError_t, (cudaEvent_t,), event)
end

function cudaEventSynchronize(event::cudaEvent_t)
    ccall((:cudaEventSynchronize, libcudnn), cudaError_t, (cudaEvent_t,), event)
end

function cudaEventDestroy(event::cudaEvent_t)
    ccall((:cudaEventDestroy, libcudnn), cudaError_t, (cudaEvent_t,), event)
end

function cudaEventElapsedTime(ms, start::cudaEvent_t, _end::cudaEvent_t)
    ccall((:cudaEventElapsedTime, libcudnn), cudaError_t, (Ptr{Cfloat}, cudaEvent_t, cudaEvent_t), ms, start, _end)
end

function cudaLaunchKernel(func, gridDim::dim3, blockDim::dim3, args, sharedMem::Cint, stream::cudaStream_t)
    ccall((:cudaLaunchKernel, libcudnn), cudaError_t, (Ptr{Void}, dim3, dim3, Ptr{Ptr{Void}}, Cint, cudaStream_t), func, gridDim, blockDim, args, sharedMem, stream)
end

function cudaFuncSetCacheConfig(func, cacheConfig::cudaFuncCache)
    ccall((:cudaFuncSetCacheConfig, libcudnn), cudaError_t, (Ptr{Void}, cudaFuncCache), func, cacheConfig)
end

function cudaFuncSetSharedMemConfig(func, config::cudaSharedMemConfig)
    ccall((:cudaFuncSetSharedMemConfig, libcudnn), cudaError_t, (Ptr{Void}, cudaSharedMemConfig), func, config)
end

function cudaFuncGetAttributes(attr, func)
    ccall((:cudaFuncGetAttributes, libcudnn), cudaError_t, (Ptr{cudaFuncAttributes}, Ptr{Void}), attr, func)
end

function cudaSetDoubleForDevice(d)
    ccall((:cudaSetDoubleForDevice, libcudnn), cudaError_t, (Ptr{Cdouble},), d)
end

function cudaSetDoubleForHost(d)
    ccall((:cudaSetDoubleForHost, libcudnn), cudaError_t, (Ptr{Cdouble},), d)
end

function cudaOccupancyMaxActiveBlocksPerMultiprocessor(numBlocks, func, blockSize::Cint, dynamicSMemSize::Cint)
    ccall((:cudaOccupancyMaxActiveBlocksPerMultiprocessor, libcudnn), cudaError_t, (Ptr{Cint}, Ptr{Void}, Cint, Cint), numBlocks, func, blockSize, dynamicSMemSize)
end
...
function cudaOccupancyMaxActiveBlocksPerMultiprocessorWithFlags(numBlocks, func, blockSize::Cint, dynamicSMemSize::Cint, flags::UInt32)
    ccall((:cudaOccupancyMaxActiveBlocksPerMultiprocessorWithFlags, libcudnn), cudaError_t, (Ptr{Cint}, Ptr{Void}, Cint, Cint, UInt32), numBlocks, func, blockSize, dynamicSMemSize, flags)
end

function cudaConfigureCall(gridDim::dim3, blockDim::dim3, sharedMem::Cint, stream::cudaStream_t)
    ccall((:cudaConfigureCall, libcudnn), cudaError_t, (dim3, dim3, Cint, cudaStream_t), gridDim, blockDim, sharedMem, stream)
end

function cudaSetupArgument(arg, size::Cint, offset::Cint)
    ccall((:cudaSetupArgument, libcudnn), cudaError_t, (Ptr{Void}, Cint, Cint), arg, size, offset)
end

function cudaLaunch(func)
    ccall((:cudaLaunch, libcudnn), cudaError_t, (Ptr{Void},), func)
end

function cudaMallocManaged(devPtr, size::Cint, flags::UInt32)
    ccall((:cudaMallocManaged, libcudnn), cudaError_t, (Ptr{Ptr{Void}}, Cint, UInt32), devPtr, size, flags)
end

function cudaMalloc(devPtr, size::Cint)
    ccall((:cudaMalloc, libcudnn), cudaError_t, (Ptr{Ptr{Void}}, Cint), devPtr, size)
end

function cudaMallocHost(ptr, size::Cint)
    ccall((:cudaMallocHost, libcudnn), cudaError_t, (Ptr{Ptr{Void}}, Cint), ptr, size)
end

function cudaMallocPitch(devPtr, pitch, width::Cint, height::Cint)
    ccall((:cudaMallocPitch, libcudnn), cudaError_t, (Ptr{Ptr{Void}}, Ptr{Cint}, Cint, Cint), devPtr, pitch, width, height)
end

function cudaMallocArray(array, desc, width::Cint, height::Cint, flags::UInt32)
    ccall((:cudaMallocArray, libcudnn), cudaError_t, (Ptr{cudaArray_t}, Ptr{cudaChannelFormatDesc}, Cint, Cint, UInt32), array, desc, width, height, flags)
end

function cudaFree(devPtr)
    ccall((:cudaFree, libcudnn), cudaError_t, (Ptr{Void},), devPtr)
end

function cudaFreeHost(ptr)
    ccall((:cudaFreeHost, libcudnn), cudaError_t, (Ptr{Void},), ptr)
end

function cudaFreeArray(array::cudaArray_t)
    ccall((:cudaFreeArray, libcudnn), cudaError_t, (cudaArray_t,), array)
end

function cudaFreeMipmappedArray(mipmappedArray::cudaMipmappedArray_t)
    ccall((:cudaFreeMipmappedArray, libcudnn), cudaError_t, (cudaMipmappedArray_t,), mipmappedArray)
end

function cudaHostAlloc(pHost, size::Cint, flags::UInt32)
    ccall((:cudaHostAlloc, libcudnn), cudaError_t, (Ptr{Ptr{Void}}, Cint, UInt32), pHost, size, flags)
end

function cudaHostRegister(ptr, size::Cint, flags::UInt32)
    ccall((:cudaHostRegister, libcudnn), cudaError_t, (Ptr{Void}, Cint, UInt32), ptr, size, flags)
end

function cudaHostUnregister(ptr)
    ccall((:cudaHostUnregister, libcudnn), cudaError_t, (Ptr{Void},), ptr)
end

function cudaHostGetDevicePointer(pDevice, pHost, flags::UInt32)
    ccall((:cudaHostGetDevicePointer, libcudnn), cudaError_t, (Ptr{Ptr{Void}}, Ptr{Void}, UInt32), pDevice, pHost, flags)
end

function cudaHostGetFlags(pFlags, pHost)
    ccall((:cudaHostGetFlags, libcudnn), cudaError_t, (Ptr{UInt32}, Ptr{Void}), pFlags, pHost)
end

function cudaMalloc3D(pitchedDevPtr, extent::cudaExtent)
    ccall((:cudaMalloc3D, libcudnn), cudaError_t, (Ptr{cudaPitchedPtr}, cudaExtent), pitchedDevPtr, extent)
end

function cudaMalloc3DArray(array, desc, extent::cudaExtent, flags::UInt32)
    ccall((:cudaMalloc3DArray, libcudnn), cudaError_t, (Ptr{cudaArray_t}, Ptr{cudaChannelFormatDesc}, cudaExtent, UInt32), array, desc, extent, flags)
end

function cudaMallocMipmappedArray(mipmappedArray, desc, extent::cudaExtent, numLevels::UInt32, flags::UInt32)
    ccall((:cudaMallocMipmappedArray, libcudnn), cudaError_t, (Ptr{cudaMipmappedArray_t}, Ptr{cudaChannelFormatDesc}, cudaExtent, UInt32, UInt32), mipmappedArray, desc, extent, numLevels, flags)
end

function cudaGetMipmappedArrayLevel(levelArray, mipmappedArray::cudaMipmappedArray_const_t, level::UInt32)
    ccall((:cudaGetMipmappedArrayLevel, libcudnn), cudaError_t, (Ptr{cudaArray_t}, cudaMipmappedArray_const_t, UInt32), levelArray, mipmappedArray, level)
end

function cudaMemcpy3D(p)
    ccall((:cudaMemcpy3D, libcudnn), cudaError_t, (Ptr{cudaMemcpy3DParms},), p)
end

function cudaMemcpy3DPeer(p)
    ccall((:cudaMemcpy3DPeer, libcudnn), cudaError_t, (Ptr{cudaMemcpy3DPeerParms},), p)
end

function cudaMemcpy3DAsync(p, stream::cudaStream_t)
    ccall((:cudaMemcpy3DAsync, libcudnn), cudaError_t, (Ptr{cudaMemcpy3DParms}, cudaStream_t), p, stream)
end

function cudaMemcpy3DPeerAsync(p, stream::cudaStream_t)
    ccall((:cudaMemcpy3DPeerAsync, libcudnn), cudaError_t, (Ptr{cudaMemcpy3DPeerParms}, cudaStream_t), p, stream)
end

function cudaMemGetInfo(free, total)
    ccall((:cudaMemGetInfo, libcudnn), cudaError_t, (Ptr{Cint}, Ptr{Cint}), free, total)
end

function cudaArrayGetInfo(desc, extent, flags, array::cudaArray_t)
    ccall((:cudaArrayGetInfo, libcudnn), cudaError_t, (Ptr{cudaChannelFormatDesc}, Ptr{cudaExtent}, Ptr{UInt32}, cudaArray_t), desc, extent, flags, array)
end

function cudaMemcpy(dst, src, count::Cint, kind::cudaMemcpyKind)
    ccall((:cudaMemcpy, libcudnn), cudaError_t, (Ptr{Void}, Ptr{Void}, Cint, cudaMemcpyKind), dst, src, count, kind)
end

function cudaMemcpyPeer(dst, dstDevice::Cint, src, srcDevice::Cint, count::Cint)
    ccall((:cudaMemcpyPeer, libcudnn), cudaError_t, (Ptr{Void}, Cint, Ptr{Void}, Cint, Cint), dst, dstDevice, src, srcDevice, count)
end

function cudaMemcpyToArray(dst::cudaArray_t, wOffset::Cint, hOffset::Cint, src, count::Cint, kind::cudaMemcpyKind)
    ccall((:cudaMemcpyToArray, libcudnn), cudaError_t, (cudaArray_t, Cint, Cint, Ptr{Void}, Cint, cudaMemcpyKind), dst, wOffset, hOffset, src, count, kind)
end

function cudaMemcpyFromArray(dst, src::cudaArray_const_t, wOffset::Cint, hOffset::Cint, count::Cint, kind::cudaMemcpyKind)
    ccall((:cudaMemcpyFromArray, libcudnn), cudaError_t, (Ptr{Void}, cudaArray_const_t, Cint, Cint, Cint, cudaMemcpyKind), dst, src, wOffset, hOffset, count, kind)
end
...
function cudaMemcpyArrayToArray(dst::cudaArray_t, wOffsetDst::Cint, hOffsetDst::Cint, src::cudaArray_const_t, wOffsetSrc::Cint, hOffsetSrc::Cint, count::Cint, kind::cudaMemcpyKind)
    ccall((:cudaMemcpyArrayToArray, libcudnn), cudaError_t, (cudaArray_t, Cint, Cint, cudaArray_const_t, Cint, Cint, Cint, cudaMemcpyKind), dst, wOffsetDst, hOffsetDst, src, wOffsetSrc, hOffsetSrc, count, kind)
end

function cudaMemcpy2D(dst, dpitch::Cint, src, spitch::Cint, width::Cint, height::Cint, kind::cudaMemcpyKind)
    ccall((:cudaMemcpy2D, libcudnn), cudaError_t, (Ptr{Void}, Cint, Ptr{Void}, Cint, Cint, Cint, cudaMemcpyKind), dst, dpitch, src, spitch, width, height, kind)
end
...
function cudaMemcpy2DToArray(dst::cudaArray_t, wOffset::Cint, hOffset::Cint, src, spitch::Cint, width::Cint, height::Cint, kind::cudaMemcpyKind)
    ccall((:cudaMemcpy2DToArray, libcudnn), cudaError_t, (cudaArray_t, Cint, Cint, Ptr{Void}, Cint, Cint, Cint, cudaMemcpyKind), dst, wOffset, hOffset, src, spitch, width, height, kind)
end
...
function cudaMemcpy2DFromArray(dst, dpitch::Cint, src::cudaArray_const_t, wOffset::Cint, hOffset::Cint, width::Cint, height::Cint, kind::cudaMemcpyKind)
    ccall((:cudaMemcpy2DFromArray, libcudnn), cudaError_t, (Ptr{Void}, Cint, cudaArray_const_t, Cint, Cint, Cint, Cint, cudaMemcpyKind), dst, dpitch, src, wOffset, hOffset, width, height, kind)
end
...
function cudaMemcpy2DArrayToArray(dst::cudaArray_t, wOffsetDst::Cint, hOffsetDst::Cint, src::cudaArray_const_t, wOffsetSrc::Cint, hOffsetSrc::Cint, width::Cint, height::Cint, kind::cudaMemcpyKind)
    ccall((:cudaMemcpy2DArrayToArray, libcudnn), cudaError_t, (cudaArray_t, Cint, Cint, cudaArray_const_t, Cint, Cint, Cint, Cint, cudaMemcpyKind), dst, wOffsetDst, hOffsetDst, src, wOffsetSrc, hOffsetSrc, width, height, kind)
end

function cudaMemcpyToSymbol(symbol, src, count::Cint, offset::Cint, kind::cudaMemcpyKind)
    ccall((:cudaMemcpyToSymbol, libcudnn), cudaError_t, (Ptr{Void}, Ptr{Void}, Cint, Cint, cudaMemcpyKind), symbol, src, count, offset, kind)
end

function cudaMemcpyFromSymbol(dst, symbol, count::Cint, offset::Cint, kind::cudaMemcpyKind)
    ccall((:cudaMemcpyFromSymbol, libcudnn), cudaError_t, (Ptr{Void}, Ptr{Void}, Cint, Cint, cudaMemcpyKind), dst, symbol, count, offset, kind)
end

function cudaMemcpyAsync(dst, src, count::Cint, kind::cudaMemcpyKind, stream::cudaStream_t)
    ccall((:cudaMemcpyAsync, libcudnn), cudaError_t, (Ptr{Void}, Ptr{Void}, Cint, cudaMemcpyKind, cudaStream_t), dst, src, count, kind, stream)
end

function cudaMemcpyPeerAsync(dst, dstDevice::Cint, src, srcDevice::Cint, count::Cint, stream::cudaStream_t)
    ccall((:cudaMemcpyPeerAsync, libcudnn), cudaError_t, (Ptr{Void}, Cint, Ptr{Void}, Cint, Cint, cudaStream_t), dst, dstDevice, src, srcDevice, count, stream)
end
...
function cudaMemcpyToArrayAsync(dst::cudaArray_t, wOffset::Cint, hOffset::Cint, src, count::Cint, kind::cudaMemcpyKind, stream::cudaStream_t)
    ccall((:cudaMemcpyToArrayAsync, libcudnn), cudaError_t, (cudaArray_t, Cint, Cint, Ptr{Void}, Cint, cudaMemcpyKind, cudaStream_t), dst, wOffset, hOffset, src, count, kind, stream)
end
...
function cudaMemcpyFromArrayAsync(dst, src::cudaArray_const_t, wOffset::Cint, hOffset::Cint, count::Cint, kind::cudaMemcpyKind, stream::cudaStream_t)
    ccall((:cudaMemcpyFromArrayAsync, libcudnn), cudaError_t, (Ptr{Void}, cudaArray_const_t, Cint, Cint, Cint, cudaMemcpyKind, cudaStream_t), dst, src, wOffset, hOffset, count, kind, stream)
end
...
function cudaMemcpy2DAsync(dst, dpitch::Cint, src, spitch::Cint, width::Cint, height::Cint, kind::cudaMemcpyKind, stream::cudaStream_t)
    ccall((:cudaMemcpy2DAsync, libcudnn), cudaError_t, (Ptr{Void}, Cint, Ptr{Void}, Cint, Cint, Cint, cudaMemcpyKind, cudaStream_t), dst, dpitch, src, spitch, width, height, kind, stream)
end
...
function cudaMemcpy2DToArrayAsync(dst::cudaArray_t, wOffset::Cint, hOffset::Cint, src, spitch::Cint, width::Cint, height::Cint, kind::cudaMemcpyKind, stream::cudaStream_t)
    ccall((:cudaMemcpy2DToArrayAsync, libcudnn), cudaError_t, (cudaArray_t, Cint, Cint, Ptr{Void}, Cint, Cint, Cint, cudaMemcpyKind, cudaStream_t), dst, wOffset, hOffset, src, spitch, width, height, kind, stream)
end
...
function cudaMemcpy2DFromArrayAsync(dst, dpitch::Cint, src::cudaArray_const_t, wOffset::Cint, hOffset::Cint, width::Cint, height::Cint, kind::cudaMemcpyKind, stream::cudaStream_t)
    ccall((:cudaMemcpy2DFromArrayAsync, libcudnn), cudaError_t, (Ptr{Void}, Cint, cudaArray_const_t, Cint, Cint, Cint, Cint, cudaMemcpyKind, cudaStream_t), dst, dpitch, src, wOffset, hOffset, width, height, kind, stream)
end

function cudaMemcpyToSymbolAsync(symbol, src, count::Cint, offset::Cint, kind::cudaMemcpyKind, stream::cudaStream_t)
    ccall((:cudaMemcpyToSymbolAsync, libcudnn), cudaError_t, (Ptr{Void}, Ptr{Void}, Cint, Cint, cudaMemcpyKind, cudaStream_t), symbol, src, count, offset, kind, stream)
end

function cudaMemcpyFromSymbolAsync(dst, symbol, count::Cint, offset::Cint, kind::cudaMemcpyKind, stream::cudaStream_t)
    ccall((:cudaMemcpyFromSymbolAsync, libcudnn), cudaError_t, (Ptr{Void}, Ptr{Void}, Cint, Cint, cudaMemcpyKind, cudaStream_t), dst, symbol, count, offset, kind, stream)
end

function cudaMemset(devPtr, value::Cint, count::Cint)
    ccall((:cudaMemset, libcudnn), cudaError_t, (Ptr{Void}, Cint, Cint), devPtr, value, count)
end

function cudaMemset2D(devPtr, pitch::Cint, value::Cint, width::Cint, height::Cint)
    ccall((:cudaMemset2D, libcudnn), cudaError_t, (Ptr{Void}, Cint, Cint, Cint, Cint), devPtr, pitch, value, width, height)
end

function cudaMemset3D(pitchedDevPtr::cudaPitchedPtr, value::Cint, extent::cudaExtent)
    ccall((:cudaMemset3D, libcudnn), cudaError_t, (cudaPitchedPtr, Cint, cudaExtent), pitchedDevPtr, value, extent)
end

function cudaMemsetAsync(devPtr, value::Cint, count::Cint, stream::cudaStream_t)
    ccall((:cudaMemsetAsync, libcudnn), cudaError_t, (Ptr{Void}, Cint, Cint, cudaStream_t), devPtr, value, count, stream)
end

function cudaMemset2DAsync(devPtr, pitch::Cint, value::Cint, width::Cint, height::Cint, stream::cudaStream_t)
    ccall((:cudaMemset2DAsync, libcudnn), cudaError_t, (Ptr{Void}, Cint, Cint, Cint, Cint, cudaStream_t), devPtr, pitch, value, width, height, stream)
end

function cudaMemset3DAsync(pitchedDevPtr::cudaPitchedPtr, value::Cint, extent::cudaExtent, stream::cudaStream_t)
    ccall((:cudaMemset3DAsync, libcudnn), cudaError_t, (cudaPitchedPtr, Cint, cudaExtent, cudaStream_t), pitchedDevPtr, value, extent, stream)
end

function cudaGetSymbolAddress(devPtr, symbol)
    ccall((:cudaGetSymbolAddress, libcudnn), cudaError_t, (Ptr{Ptr{Void}}, Ptr{Void}), devPtr, symbol)
end

function cudaGetSymbolSize(size, symbol)
    ccall((:cudaGetSymbolSize, libcudnn), cudaError_t, (Ptr{Cint}, Ptr{Void}), size, symbol)
end

function cudaMemPrefetchAsync(devPtr, count::Cint, dstDevice::Cint, stream::cudaStream_t)
    ccall((:cudaMemPrefetchAsync, libcudnn), cudaError_t, (Ptr{Void}, Cint, Cint, cudaStream_t), devPtr, count, dstDevice, stream)
end

function cudaMemAdvise(devPtr, count::Cint, advice::cudaMemoryAdvise, device::Cint)
    ccall((:cudaMemAdvise, libcudnn), cudaError_t, (Ptr{Void}, Cint, cudaMemoryAdvise, Cint), devPtr, count, advice, device)
end

function cudaMemRangeGetAttribute(data, dataSize::Cint, attribute::cudaMemRangeAttribute, devPtr, count::Cint)
    ccall((:cudaMemRangeGetAttribute, libcudnn), cudaError_t, (Ptr{Void}, Cint, cudaMemRangeAttribute, Ptr{Void}, Cint), data, dataSize, attribute, devPtr, count)
end

function cudaMemRangeGetAttributes(data, dataSizes, attributes, numAttributes::Cint, devPtr, count::Cint)
    ccall((:cudaMemRangeGetAttributes, libcudnn), cudaError_t, (Ptr{Ptr{Void}}, Ptr{Cint}, Ptr{cudaMemRangeAttribute}, Cint, Ptr{Void}, Cint), data, dataSizes, attributes, numAttributes, devPtr, count)
end

function cudaPointerGetAttributes(attributes, ptr)
    ccall((:cudaPointerGetAttributes, libcudnn), cudaError_t, (Ptr{cudaPointerAttributes}, Ptr{Void}), attributes, ptr)
end

function cudaDeviceCanAccessPeer(canAccessPeer, device::Cint, peerDevice::Cint)
    ccall((:cudaDeviceCanAccessPeer, libcudnn), cudaError_t, (Ptr{Cint}, Cint, Cint), canAccessPeer, device, peerDevice)
end

function cudaDeviceEnablePeerAccess(peerDevice::Cint, flags::UInt32)
    ccall((:cudaDeviceEnablePeerAccess, libcudnn), cudaError_t, (Cint, UInt32), peerDevice, flags)
end

function cudaDeviceDisablePeerAccess(peerDevice::Cint)
    ccall((:cudaDeviceDisablePeerAccess, libcudnn), cudaError_t, (Cint,), peerDevice)
end

function cudaGraphicsUnregisterResource(resource::cudaGraphicsResource_t)
    ccall((:cudaGraphicsUnregisterResource, libcudnn), cudaError_t, (cudaGraphicsResource_t,), resource)
end

function cudaGraphicsResourceSetMapFlags(resource::cudaGraphicsResource_t, flags::UInt32)
    ccall((:cudaGraphicsResourceSetMapFlags, libcudnn), cudaError_t, (cudaGraphicsResource_t, UInt32), resource, flags)
end

function cudaGraphicsMapResources(count::Cint, resources, stream::cudaStream_t)
    ccall((:cudaGraphicsMapResources, libcudnn), cudaError_t, (Cint, Ptr{cudaGraphicsResource_t}, cudaStream_t), count, resources, stream)
end

function cudaGraphicsUnmapResources(count::Cint, resources, stream::cudaStream_t)
    ccall((:cudaGraphicsUnmapResources, libcudnn), cudaError_t, (Cint, Ptr{cudaGraphicsResource_t}, cudaStream_t), count, resources, stream)
end

function cudaGraphicsResourceGetMappedPointer(devPtr, size, resource::cudaGraphicsResource_t)
    ccall((:cudaGraphicsResourceGetMappedPointer, libcudnn), cudaError_t, (Ptr{Ptr{Void}}, Ptr{Cint}, cudaGraphicsResource_t), devPtr, size, resource)
end

function cudaGraphicsSubResourceGetMappedArray(array, resource::cudaGraphicsResource_t, arrayIndex::UInt32, mipLevel::UInt32)
    ccall((:cudaGraphicsSubResourceGetMappedArray, libcudnn), cudaError_t, (Ptr{cudaArray_t}, cudaGraphicsResource_t, UInt32, UInt32), array, resource, arrayIndex, mipLevel)
end

function cudaGraphicsResourceGetMappedMipmappedArray(mipmappedArray, resource::cudaGraphicsResource_t)
    ccall((:cudaGraphicsResourceGetMappedMipmappedArray, libcudnn), cudaError_t, (Ptr{cudaMipmappedArray_t}, cudaGraphicsResource_t), mipmappedArray, resource)
end

function cudaGetChannelDesc(desc, array::cudaArray_const_t)
    ccall((:cudaGetChannelDesc, libcudnn), cudaError_t, (Ptr{cudaChannelFormatDesc}, cudaArray_const_t), desc, array)
end

function cudaCreateChannelDesc(x::Cint, y::Cint, z::Cint, w::Cint, f::cudaChannelFormatKind)
    ccall((:cudaCreateChannelDesc, libcudnn), cudaChannelFormatDesc, (Cint, Cint, Cint, Cint, cudaChannelFormatKind), x, y, z, w, f)
end

function cudaBindTexture(offset, texref, devPtr, desc, size::Cint)
    ccall((:cudaBindTexture, libcudnn), cudaError_t, (Ptr{Cint}, Ptr{textureReference}, Ptr{Void}, Ptr{cudaChannelFormatDesc}, Cint), offset, texref, devPtr, desc, size)
end

function cudaBindTexture2D(offset, texref, devPtr, desc, width::Cint, height::Cint, pitch::Cint)
    ccall((:cudaBindTexture2D, libcudnn), cudaError_t, (Ptr{Cint}, Ptr{textureReference}, Ptr{Void}, Ptr{cudaChannelFormatDesc}, Cint, Cint, Cint), offset, texref, devPtr, desc, width, height, pitch)
end

function cudaBindTextureToArray(texref, array::cudaArray_const_t, desc)
    ccall((:cudaBindTextureToArray, libcudnn), cudaError_t, (Ptr{textureReference}, cudaArray_const_t, Ptr{cudaChannelFormatDesc}), texref, array, desc)
end

function cudaBindTextureToMipmappedArray(texref, mipmappedArray::cudaMipmappedArray_const_t, desc)
    ccall((:cudaBindTextureToMipmappedArray, libcudnn), cudaError_t, (Ptr{textureReference}, cudaMipmappedArray_const_t, Ptr{cudaChannelFormatDesc}), texref, mipmappedArray, desc)
end

function cudaUnbindTexture(texref)
    ccall((:cudaUnbindTexture, libcudnn), cudaError_t, (Ptr{textureReference},), texref)
end

function cudaGetTextureAlignmentOffset(offset, texref)
    ccall((:cudaGetTextureAlignmentOffset, libcudnn), cudaError_t, (Ptr{Cint}, Ptr{textureReference}), offset, texref)
end

function cudaGetTextureReference(texref, symbol)
    ccall((:cudaGetTextureReference, libcudnn), cudaError_t, (Ptr{Ptr{textureReference}}, Ptr{Void}), texref, symbol)
end

function cudaBindSurfaceToArray(surfref, array::cudaArray_const_t, desc)
    ccall((:cudaBindSurfaceToArray, libcudnn), cudaError_t, (Ptr{surfaceReference}, cudaArray_const_t, Ptr{cudaChannelFormatDesc}), surfref, array, desc)
end

function cudaGetSurfaceReference(surfref, symbol)
    ccall((:cudaGetSurfaceReference, libcudnn), cudaError_t, (Ptr{Ptr{surfaceReference}}, Ptr{Void}), surfref, symbol)
end

function cudaCreateTextureObject(pTexObject, pResDesc, pTexDesc, pResViewDesc)
    ccall((:cudaCreateTextureObject, libcudnn), cudaError_t, (Ptr{cudaTextureObject_t}, Ptr{cudaResourceDesc}, Ptr{cudaTextureDesc}, Ptr{cudaResourceViewDesc}), pTexObject, pResDesc, pTexDesc, pResViewDesc)
end

function cudaDestroyTextureObject(texObject::cudaTextureObject_t)
    ccall((:cudaDestroyTextureObject, libcudnn), cudaError_t, (cudaTextureObject_t,), texObject)
end

function cudaGetTextureObjectResourceDesc(pResDesc, texObject::cudaTextureObject_t)
    ccall((:cudaGetTextureObjectResourceDesc, libcudnn), cudaError_t, (Ptr{cudaResourceDesc}, cudaTextureObject_t), pResDesc, texObject)
end

function cudaGetTextureObjectTextureDesc(pTexDesc, texObject::cudaTextureObject_t)
    ccall((:cudaGetTextureObjectTextureDesc, libcudnn), cudaError_t, (Ptr{cudaTextureDesc}, cudaTextureObject_t), pTexDesc, texObject)
end

function cudaGetTextureObjectResourceViewDesc(pResViewDesc, texObject::cudaTextureObject_t)
    ccall((:cudaGetTextureObjectResourceViewDesc, libcudnn), cudaError_t, (Ptr{cudaResourceViewDesc}, cudaTextureObject_t), pResViewDesc, texObject)
end

function cudaCreateSurfaceObject(pSurfObject, pResDesc)
    ccall((:cudaCreateSurfaceObject, libcudnn), cudaError_t, (Ptr{cudaSurfaceObject_t}, Ptr{cudaResourceDesc}), pSurfObject, pResDesc)
end

function cudaDestroySurfaceObject(surfObject::cudaSurfaceObject_t)
    ccall((:cudaDestroySurfaceObject, libcudnn), cudaError_t, (cudaSurfaceObject_t,), surfObject)
end

function cudaGetSurfaceObjectResourceDesc(pResDesc, surfObject::cudaSurfaceObject_t)
    ccall((:cudaGetSurfaceObjectResourceDesc, libcudnn), cudaError_t, (Ptr{cudaResourceDesc}, cudaSurfaceObject_t), pResDesc, surfObject)
end

function cudaDriverGetVersion(driverVersion)
    ccall((:cudaDriverGetVersion, libcudnn), cudaError_t, (Ptr{Cint},), driverVersion)
end

function cudaRuntimeGetVersion(runtimeVersion)
    ccall((:cudaRuntimeGetVersion, libcudnn), cudaError_t, (Ptr{Cint},), runtimeVersion)
end

function cudaGetExportTable(ppExportTable, pExportTableId)
    ccall((:cudaGetExportTable, libcudnn), cudaError_t, (Ptr{Ptr{Void}}, Ptr{cudaUUID_t}), ppExportTable, pExportTableId)
end

function make_cudaPitchedPtr(d, p::Cint, xsz::Cint, ysz::Cint)
    ccall((:make_cudaPitchedPtr, libcudnn), cudaPitchedPtr, (Ptr{Void}, Cint, Cint, Cint), d, p, xsz, ysz)
end

function make_cudaPos(x::Cint, y::Cint, z::Cint)
    ccall((:make_cudaPos, libcudnn), cudaPos, (Cint, Cint, Cint), x, y, z)
end

function make_cudaExtent(w::Cint, h::Cint, d::Cint)
    ccall((:make_cudaExtent, libcudnn), cudaExtent, (Cint, Cint, Cint), w, h, d)
end

function make_char1(x::UInt8)
    ccall((:make_char1, libcudnn), char1, (UInt8,), x)
end

function make_uchar1(x::Cuchar)
    ccall((:make_uchar1, libcudnn), uchar1, (Cuchar,), x)
end

# function make_char2(x::UInt8, y::UInt8)
#     ccall((:make_char2, libcudnn), char2, (UInt8, UInt8), x, y)
# end

# function make_uchar2(x::Cuchar, y::Cuchar)
#     ccall((:make_uchar2, libcudnn), uchar2, (Cuchar, Cuchar), x, y)
# end

# function make_char3(x::UInt8, y::UInt8, z::UInt8)
#     ccall((:make_char3, libcudnn), char3, (UInt8, UInt8, UInt8), x, y, z)
# end

function make_uchar3(x::Cuchar, y::Cuchar, z::Cuchar)
    ccall((:make_uchar3, libcudnn), uchar3, (Cuchar, Cuchar, Cuchar), x, y, z)
end

# function make_char4(x::UInt8, y::UInt8, z::UInt8, w::UInt8)
#     ccall((:make_char4, libcudnn), char4, (UInt8, UInt8, UInt8, UInt8), x, y, z, w)
# end

# function make_uchar4(x::Cuchar, y::Cuchar, z::Cuchar, w::Cuchar)
#     ccall((:make_uchar4, libcudnn), uchar4, (Cuchar, Cuchar, Cuchar, Cuchar), x, y, z, w)
# end

function make_short1(x::Int16)
    ccall((:make_short1, libcudnn), short1, (Int16,), x)
end

function make_ushort1(x::UInt16)
    ccall((:make_ushort1, libcudnn), ushort1, (UInt16,), x)
end

# function make_short2(x::Int16, y::Int16)
#     ccall((:make_short2, libcudnn), short2, (Int16, Int16), x, y)
# end

# function make_ushort2(x::UInt16, y::UInt16)
#     ccall((:make_ushort2, libcudnn), ushort2, (UInt16, UInt16), x, y)
# end

# function make_short3(x::Int16, y::Int16, z::Int16)
#     ccall((:make_short3, libcudnn), short3, (Int16, Int16, Int16), x, y, z)
# end

# function make_ushort3(x::UInt16, y::UInt16, z::UInt16)
#     ccall((:make_ushort3, libcudnn), ushort3, (UInt16, UInt16, UInt16), x, y, z)
# end

# function make_short4(x::Int16, y::Int16, z::Int16, w::Int16)
#     ccall((:make_short4, libcudnn), short4, (Int16, Int16, Int16, Int16), x, y, z, w)
# end

# function make_ushort4(x::UInt16, y::UInt16, z::UInt16, w::UInt16)
#     ccall((:make_ushort4, libcudnn), ushort4, (UInt16, UInt16, UInt16, UInt16), x, y, z, w)
# end

# function make_int1(x::Cint)
#     ccall((:make_int1, libcudnn), int1, (Cint,), x)
# end

# function make_uint1(x::UInt32)
#     ccall((:make_uint1, libcudnn), uint1, (UInt32,), x)
# end

# function make_int2(x::Cint, y::Cint)
#     ccall((:make_int2, libcudnn), int2, (Cint, Cint), x, y)
# end

# function make_uint2(x::UInt32, y::UInt32)
#     ccall((:make_uint2, libcudnn), uint2, (UInt32, UInt32), x, y)
# end

# function make_int3(x::Cint, y::Cint, z::Cint)
#     ccall((:make_int3, libcudnn), int3, (Cint, Cint, Cint), x, y, z)
# end

# function make_uint3(x::UInt32, y::UInt32, z::UInt32)
#     ccall((:make_uint3, libcudnn), uint3, (UInt32, UInt32, UInt32), x, y, z)
# end

# function make_int4(x::Cint, y::Cint, z::Cint, w::Cint)
#     ccall((:make_int4, libcudnn), int4, (Cint, Cint, Cint, Cint), x, y, z, w)
# end

# function make_uint4(x::UInt32, y::UInt32, z::UInt32, w::UInt32)
#     ccall((:make_uint4, libcudnn), uint4, (UInt32, UInt32, UInt32, UInt32), x, y, z, w)
# end

# function make_long1(x::Clong)
#     ccall((:make_long1, libcudnn), long1, (Clong,), x)
# end

# function make_ulong1(x::Culong)
#     ccall((:make_ulong1, libcudnn), ulong1, (Culong,), x)
# end

# function make_long2(x::Clong, y::Clong)
#     ccall((:make_long2, libcudnn), long2, (Clong, Clong), x, y)
# end

# function make_ulong2(x::Culong, y::Culong)
#     ccall((:make_ulong2, libcudnn), ulong2, (Culong, Culong), x, y)
# end

# function make_long3(x::Clong, y::Clong, z::Clong)
#     ccall((:make_long3, libcudnn), long3, (Clong, Clong, Clong), x, y, z)
# end

# function make_ulong3(x::Culong, y::Culong, z::Culong)
#     ccall((:make_ulong3, libcudnn), ulong3, (Culong, Culong, Culong), x, y, z)
# end

# function make_long4(x::Clong, y::Clong, z::Clong, w::Clong)
#     ccall((:make_long4, libcudnn), long4, (Clong, Clong, Clong, Clong), x, y, z, w)
# end

# function make_ulong4(x::Culong, y::Culong, z::Culong, w::Culong)
#     ccall((:make_ulong4, libcudnn), ulong4, (Culong, Culong, Culong, Culong), x, y, z, w)
# end

# function make_float1(x::Cfloat)
#     ccall((:make_float1, libcudnn), float1, (Cfloat,), x)
# end

# function make_float2(x::Cfloat, y::Cfloat)
#     ccall((:make_float2, libcudnn), float2, (Cfloat, Cfloat), x, y)
# end

# function make_float3(x::Cfloat, y::Cfloat, z::Cfloat)
#     ccall((:make_float3, libcudnn), float3, (Cfloat, Cfloat, Cfloat), x, y, z)
# end

# function make_float4(x::Cfloat, y::Cfloat, z::Cfloat, w::Cfloat)
#     ccall((:make_float4, libcudnn), float4, (Cfloat, Cfloat, Cfloat, Cfloat), x, y, z, w)
# end

# function make_longlong1(x::Clonglong)
#     ccall((:make_longlong1, libcudnn), longlong1, (Clonglong,), x)
# end

# function make_ulonglong1(x::Culonglong)
#     ccall((:make_ulonglong1, libcudnn), ulonglong1, (Culonglong,), x)
# end

# function make_longlong2(x::Clonglong, y::Clonglong)
#     ccall((:make_longlong2, libcudnn), longlong2, (Clonglong, Clonglong), x, y)
# end

# function make_ulonglong2(x::Culonglong, y::Culonglong)
#     ccall((:make_ulonglong2, libcudnn), ulonglong2, (Culonglong, Culonglong), x, y)
# end

# function make_longlong3(x::Clonglong, y::Clonglong, z::Clonglong)
#     ccall((:make_longlong3, libcudnn), longlong3, (Clonglong, Clonglong, Clonglong), x, y, z)
# end

# function make_ulonglong3(x::Culonglong, y::Culonglong, z::Culonglong)
#     ccall((:make_ulonglong3, libcudnn), ulonglong3, (Culonglong, Culonglong, Culonglong), x, y, z)
# end

# function make_longlong4(x::Clonglong, y::Clonglong, z::Clonglong, w::Clonglong)
#     ccall((:make_longlong4, libcudnn), longlong4, (Clonglong, Clonglong, Clonglong, Clonglong), x, y, z, w)
# end

# function make_ulonglong4(x::Culonglong, y::Culonglong, z::Culonglong, w::Culonglong)
#     ccall((:make_ulonglong4, libcudnn), ulonglong4, (Culonglong, Culonglong, Culonglong, Culonglong), x, y, z, w)
# end

# function make_double1(x::Cdouble)
#     ccall((:make_double1, libcudnn), double1, (Cdouble,), x)
# end

# function make_double2(x::Cdouble, y::Cdouble)
#     ccall((:make_double2, libcudnn), double2, (Cdouble, Cdouble), x, y)
# end

# function make_double3(x::Cdouble, y::Cdouble, z::Cdouble)
#     ccall((:make_double3, libcudnn), double3, (Cdouble, Cdouble, Cdouble), x, y, z)
# end

# function make_double4(x::Cdouble, y::Cdouble, z::Cdouble, w::Cdouble)
#     ccall((:make_double4, libcudnn), double4, (Cdouble, Cdouble, Cdouble, Cdouble), x, y, z, w)
# end
...
function cudnnSetConvolution2dDescriptor_v5(convDesc::cudnnConvolutionDescriptor_t, pad_h::Cint, pad_w::Cint, u::Cint, v::Cint, upscalex::Cint, upscaley::Cint, mode::cudnnConvolutionMode_t, dataType::cudnnDataType_t)
    ccall((:cudnnSetConvolution2dDescriptor_v5, libcudnn), cudnnStatus_t, (cudnnConvolutionDescriptor_t, Cint, Cint, Cint, Cint, Cint, Cint, cudnnConvolutionMode_t, cudnnDataType_t), convDesc, pad_h, pad_w, u, v, upscalex, upscaley, mode, dataType)
end
...
function cudnnGetConvolution2dDescriptor_v5(convDesc::cudnnConvolutionDescriptor_t, pad_h, pad_w, u, v, upscalex, upscaley, mode, dataType)
    ccall((:cudnnGetConvolution2dDescriptor_v5, libcudnn), cudnnStatus_t, (cudnnConvolutionDescriptor_t, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}, Ptr{cudnnConvolutionMode_t}, Ptr{cudnnDataType_t}), convDesc, pad_h, pad_w, u, v, upscalex, upscaley, mode, dataType)
end
...
function cudnnDropoutGetStatesSize(handle::cudnnHandle_t, sizeInBytes)
    ccall((:cudnnDropoutGetStatesSize, libcudnn), cudnnStatus_t, (cudnnHandle_t, Ptr{Cint}), handle, sizeInBytes)
end

function cudnnDropoutGetReserveSpaceSize(xdesc::cudnnTensorDescriptor_t, sizeInBytes)
    ccall((:cudnnDropoutGetReserveSpaceSize, libcudnn), cudnnStatus_t, (cudnnTensorDescriptor_t, Ptr{Cint}), xdesc, sizeInBytes)
end
...
function cudnnSetDropoutDescriptor(dropoutDesc::cudnnDropoutDescriptor_t, handle::cudnnHandle_t, dropout::Cfloat, states, stateSizeInBytes::Cint, seed::Culonglong)
    ccall((:cudnnSetDropoutDescriptor, libcudnn), cudnnStatus_t, (cudnnDropoutDescriptor_t, cudnnHandle_t, Cfloat, Ptr{Void}, Cint, Culonglong), dropoutDesc, handle, dropout, states, stateSizeInBytes, seed)
end
...
function cudnnDropoutForward(handle::cudnnHandle_t, dropoutDesc::cudnnDropoutDescriptor_t, xdesc::cudnnTensorDescriptor_t, x, ydesc::cudnnTensorDescriptor_t, y, reserveSpace, reserveSpaceSizeInBytes::Cint)
    ccall((:cudnnDropoutForward, libcudnn), cudnnStatus_t, (cudnnHandle_t, cudnnDropoutDescriptor_t, cudnnTensorDescriptor_t, Ptr{Void}, cudnnTensorDescriptor_t, Ptr{Void}, Ptr{Void}, Cint), handle, dropoutDesc, xdesc, x, ydesc, y, reserveSpace, reserveSpaceSizeInBytes)
end
...
function cudnnDropoutBackward(handle::cudnnHandle_t, dropoutDesc::cudnnDropoutDescriptor_t, dydesc::cudnnTensorDescriptor_t, dy, dxdesc::cudnnTensorDescriptor_t, dx, reserveSpace, reserveSpaceSizeInBytes::Cint)
    ccall((:cudnnDropoutBackward, libcudnn), cudnnStatus_t, (cudnnHandle_t, cudnnDropoutDescriptor_t, cudnnTensorDescriptor_t, Ptr{Void}, cudnnTensorDescriptor_t, Ptr{Void}, Ptr{Void}, Cint), handle, dropoutDesc, dydesc, dy, dxdesc, dx, reserveSpace, reserveSpaceSizeInBytes)
end
...
function cudnnSetRNNDescriptor(rnnDesc::cudnnRNNDescriptor_t, hiddenSize::Cint, numLayers::Cint, dropoutDesc::cudnnDropoutDescriptor_t, inputMode::cudnnRNNInputMode_t, direction::cudnnDirectionMode_t, mode::cudnnRNNMode_t, dataType::cudnnDataType_t)
    ccall((:cudnnSetRNNDescriptor, libcudnn), cudnnStatus_t, (cudnnRNNDescriptor_t, Cint, Cint, cudnnDropoutDescriptor_t, cudnnRNNInputMode_t, cudnnDirectionMode_t, cudnnRNNMode_t, cudnnDataType_t), rnnDesc, hiddenSize, numLayers, dropoutDesc, inputMode, direction, mode, dataType)
end

function cudnnGetRNNWorkspaceSize(handle::cudnnHandle_t, rnnDesc::cudnnRNNDescriptor_t, seqLength::Cint, xDesc, sizeInBytes)
    ccall((:cudnnGetRNNWorkspaceSize, libcudnn), cudnnStatus_t, (cudnnHandle_t, cudnnRNNDescriptor_t, Cint, Ptr{cudnnTensorDescriptor_t}, Ptr{Cint}), handle, rnnDesc, seqLength, xDesc, sizeInBytes)
end
...
function cudnnGetRNNTrainingReserveSize(handle::cudnnHandle_t, rnnDesc::cudnnRNNDescriptor_t, seqLength::Cint, xDesc, sizeInBytes)
    ccall((:cudnnGetRNNTrainingReserveSize, libcudnn), cudnnStatus_t, (cudnnHandle_t, cudnnRNNDescriptor_t, Cint, Ptr{cudnnTensorDescriptor_t}, Ptr{Cint}), handle, rnnDesc, seqLength, xDesc, sizeInBytes)
end
...
function cudnnGetRNNParamsSize(handle::cudnnHandle_t, rnnDesc::cudnnRNNDescriptor_t, xDesc::cudnnTensorDescriptor_t, sizeInBytes, dataType::cudnnDataType_t)
    ccall((:cudnnGetRNNParamsSize, libcudnn), cudnnStatus_t, (cudnnHandle_t, cudnnRNNDescriptor_t, cudnnTensorDescriptor_t, Ptr{Cint}, cudnnDataType_t), handle, rnnDesc, xDesc, sizeInBytes, dataType)
end
...
function cudnnSetFilter4dDescriptor_v3(filterDesc::cudnnFilterDescriptor_t, dataType::cudnnDataType_t, k::Cint, c::Cint, h::Cint, w::Cint)
    ccall((:cudnnSetFilter4dDescriptor_v3, libcudnn), cudnnStatus_t, (cudnnFilterDescriptor_t, cudnnDataType_t, Cint, Cint, Cint, Cint), filterDesc, dataType, k, c, h, w)
end
...
function cudnnGetFilter4dDescriptor_v3(filterDesc::cudnnFilterDescriptor_t, dataType, k, c, h, w)
    ccall((:cudnnGetFilter4dDescriptor_v3, libcudnn), cudnnStatus_t, (cudnnFilterDescriptor_t, Ptr{cudnnDataType_t}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}), filterDesc, dataType, k, c, h, w)
end
...
function cudnnSetFilterNdDescriptor_v3(filterDesc::cudnnFilterDescriptor_t, dataType::cudnnDataType_t, nbDims::Cint, filterDimA)
    ccall((:cudnnSetFilterNdDescriptor_v3, libcudnn), cudnnStatus_t, (cudnnFilterDescriptor_t, cudnnDataType_t, Cint, Ptr{Cint}), filterDesc, dataType, nbDims, filterDimA)
end
...
function cudnnGetFilterNdDescriptor_v3(filterDesc::cudnnFilterDescriptor_t, nbDimsRequested::Cint, dataType, nbDims, filterDimA)
    ccall((:cudnnGetFilterNdDescriptor_v3, libcudnn), cudnnStatus_t, (cudnnFilterDescriptor_t, Cint, Ptr{cudnnDataType_t}, Ptr{Cint}, Ptr{Cint}), filterDesc, nbDimsRequested, dataType, nbDims, filterDimA)
end
...
function cudnnSetPooling2dDescriptor_v3(poolingDesc::cudnnPoolingDescriptor_t, mode::cudnnPoolingMode_t, windowHeight::Cint, windowWidth::Cint, verticalPadding::Cint, horizontalPadding::Cint, verticalStride::Cint, horizontalStride::Cint)
    ccall((:cudnnSetPooling2dDescriptor_v3, libcudnn), cudnnStatus_t, (cudnnPoolingDescriptor_t, cudnnPoolingMode_t, Cint, Cint, Cint, Cint, Cint, Cint), poolingDesc, mode, windowHeight, windowWidth, verticalPadding, horizontalPadding, verticalStride, horizontalStride)
end
...
function cudnnGetPooling2dDescriptor_v3(poolingDesc::cudnnPoolingDescriptor_t, mode, windowHeight, windowWidth, verticalPadding, horizontalPadding, verticalStride, horizontalStride)
    ccall((:cudnnGetPooling2dDescriptor_v3, libcudnn), cudnnStatus_t, (cudnnPoolingDescriptor_t, Ptr{cudnnPoolingMode_t}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}), poolingDesc, mode, windowHeight, windowWidth, verticalPadding, horizontalPadding, verticalStride, horizontalStride)
end
...
function cudnnSetPoolingNdDescriptor_v3(poolingDesc::cudnnPoolingDescriptor_t, mode::cudnnPoolingMode_t, nbDims::Cint, windowDimA, paddingA, strideA)
    ccall((:cudnnSetPoolingNdDescriptor_v3, libcudnn), cudnnStatus_t, (cudnnPoolingDescriptor_t, cudnnPoolingMode_t, Cint, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}), poolingDesc, mode, nbDims, windowDimA, paddingA, strideA)
end
...
function cudnnGetPoolingNdDescriptor_v3(poolingDesc::cudnnPoolingDescriptor_t, nbDimsRequested::Cint, mode, nbDims, windowDimA, paddingA, strideA)
    ccall((:cudnnGetPoolingNdDescriptor_v3, libcudnn), cudnnStatus_t, (cudnnPoolingDescriptor_t, Cint, Ptr{cudnnPoolingMode_t}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}, Ptr{Cint}), poolingDesc, nbDimsRequested, mode, nbDims, windowDimA, paddingA, strideA)
end
...
function cudnnActivationForward_v3(handle::cudnnHandle_t, mode::cudnnActivationMode_t, alpha, xDesc::cudnnTensorDescriptor_t, x, beta, yDesc::cudnnTensorDescriptor_t, y)
    ccall((:cudnnActivationForward_v3, libcudnn), cudnnStatus_t, (cudnnHandle_t, cudnnActivationMode_t, Ptr{Void}, cudnnTensorDescriptor_t, Ptr{Void}, Ptr{Void}, cudnnTensorDescriptor_t, Ptr{Void}), handle, mode, alpha, xDesc, x, beta, yDesc, y)
end
...
function cudnnActivationBackward_v3(handle::cudnnHandle_t, mode::cudnnActivationMode_t, alpha, yDesc::cudnnTensorDescriptor_t, y, dyDesc::cudnnTensorDescriptor_t, dy, xDesc::cudnnTensorDescriptor_t, x, beta, dxDesc::cudnnTensorDescriptor_t, dx)
    ccall((:cudnnActivationBackward_v3, libcudnn), cudnnStatus_t, (cudnnHandle_t, cudnnActivationMode_t, Ptr{Void}, cudnnTensorDescriptor_t, Ptr{Void}, cudnnTensorDescriptor_t, Ptr{Void}, cudnnTensorDescriptor_t, Ptr{Void}, Ptr{Void}, cudnnTensorDescriptor_t, Ptr{Void}), handle, mode, alpha, yDesc, y, dyDesc, dy, xDesc, x, beta, dxDesc, dx)
end
