...
function pool_back_x{T}(x::CuArray{T}, y::CuArray{T}, dy::CuArray{T};
    ...
    @cuda(cudnn,cudnnPoolingBackward,
          ...
          TD(dy), dy.ptr, TD(x), x.ptr, Ref(T(beta)), TD(dx), dx)
    return dx
end
...
function unpool_back_x(dy; window=2, alpha=1, o...) # padding=0, stride=window, mode=0,
                                                    # maxpoolingNanOpt=0
